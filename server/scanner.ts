...
export const scanCryptoMarket=async(products:string[]=SCAN_PRODUCTS)=>{
  ...
  for(const productId of products){
    try{
      ...
      if(latest.close>sma20){score+=10;reasons.push('price above SMA20')}else score-=10
      if(sma20>sma50){score+=15;reasons.push('SMA20 above SMA50')}else score-=15
      if(change1hPercent>0){score+=5;reasons.push('positive 1h momentum')}else score-=5
      if(change24hPercent>0){score+=10;reasons.push('positive 24h momentum')}else score-=10
      if(volumeRatio>=1.05){score+=8;reasons.push('volume confirmation')}
