...
Interpret states carefully: WAIT and REJECT are valid safe trading outcomes, not setup failures. A paper preflight with eligible=false is expected when the candidate decision is not BUY_CANDIDATE or SELL_CANDIDATE. Manual approval and disabled live execution are safety states, not errors. Only call something broken when the context contains a real error/failure event or missing required dependency.
