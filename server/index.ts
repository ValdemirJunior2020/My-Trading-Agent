...
const distDir=join(process.cwd(),'dist'), pidFile=join(config.dataDir,'server.pid'), startedAt=new Date().toISOString()
...
const serveStatic=(pathname:string,res:ServerResponse)=>{if(!existsSync(distDir))return false;const clean=normalize(decodeURIComponent(pathname)).replace(/^([.][.][/\\])+/,''),candidate=join(distDir,clean==='/'?'index.html':clean),file=existsSync(candidate)&&!candidate.endsWith('/')?candidate:join(distDir,'index.html');if(!existsSync(file))return false;res.writeHead(200,{'Content-Type':mime[extname(file)]||'application/octet-stream','X-Content-Type-Options':'nosniff','Referrer-Policy':'strict-origin-when-cross-origin','X-Frame-Options':'SAMEORIGIN'});res.end(readFileSync(file));return true}
const statusPayload=async()=>({server:{online:true,version:'0.2.0',startedAt},ollama:await getOllamaStatus(),coinbase:{configured:coinbaseConfigured()},safety:{emergencyStop:emergencyStopActive(),mode:config.tradingMode,liveTradingEnabled:config.liveTradingEnabled,automaticTradingEnabled:config.autoTradingEnabled,manualApprovalRequired:config.manualApprovalRequired}})
