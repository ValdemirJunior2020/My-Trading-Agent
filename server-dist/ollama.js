...
const isLikelyEmbeddingModel = (name) => /embed|embedding|nomic-embed|bge-|e5-|snowflake-arctic-embed/i.test(name);
const canChat = async (model) => {
    try {
...
const resolveChatModel = async (models) => {
    if (config.ollamaModel) {
        if (await canChat(config.ollamaModel))
...
const ollamaError = async (response) => {
    let detail = '';
    try {
        const data = await response.json();
        detail = data.error || '';
...
export const runAgent = async (agentId, asset, summary) => {
...
decision must be one of BUY_CANDIDATE, SELL_CANDIDATE, HOLD, WAIT, REJECT.`;
    ...
    if (!response.ok)
        throw new Error(await ollamaError(response));
    const data = await response.json();
    ...
    try {
        ...
        return { model, output: { status: 'error', summary: raw, decision: 'WAIT' } };
    }
};
export const runToolCopilot = async (message, context, language = 'en') => {
    const model = await getRequiredChatModel();
...
export const runToolCopilotPlanner = async (message, context, language = 'en') => {
    const model = await getRequiredChatModel();
...
- Return JSON only, no markdown.`;
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object' || !('action' in parsed))
            return { action: 'NONE' };
        if (!['NONE', 'SET_CHALLENGE', 'RUN_AGENTS', 'EMERGENCY_STOP', 'SET_RISK_LIMITS', 'SET_AUTO_RUN'].includes(String(parsed.action)))
            return { action: 'NONE' };
        return parsed;
    }
    catch {
