...
import { emergencyStopActive, evaluateLiveOrder, evaluatePaperOrder, getDailyEquityGuard, getRuntimeRiskLimits, saveRuntimeRiskLimits } from './risk.js';
...
const readJson = async (req) => { const chunks = []; let size = 0; for await (const chunk of req) {
...
const mime = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.json': 'application/json; charset=utf-8', '.svg': 'image/svg+xml', '.png': 'image/png', '.ico': 'image/x-icon', '.webp': 'image/webp', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg' };
...
const server = createServer(async (req, res) => {
    try {
        ...
        if (path === '/api/health' && req.method === 'GET')
            return json(res, 200, { ok: true, startedAt });
        if (path === '/api/status' && req.method === 'GET')
            return json(res, 200, await statusPayload());
        if (path === '/api/events' && req.method === 'GET')
            return attachEventStream(res);
        if (path === '/api/events/recent' && req.method === 'GET')
            return json(res, 200, { events: recentEvents(30) });
        if (path === '/api/paper/orders' && req.method === 'GET')
            return json(res, 200, { orders: listPaperTrades() });
        if (path === '/api/emergency-stop' && req.method === 'POST') {
            const body = await readJson(req);
            const active = Boolean(body.active);
            setSetting('emergency_stop', String(active));
            const event = publish(active ? 'emergency_stop_activated' : 'emergency_stop_cleared', { active }, 'manager');
            return json(res, 200, { ok: true, active, event });
        }
        if (path === '/api/paper/orders' && req.method === 'POST') {
            const body = await readJson(req);
            const order = { productId: String(body.productId || '').toUpperCase(), side: String(body.side || '').toUpperCase(), size: Number(body.size), price: Number(body.price) };
            const risk = evaluatePaperOrder(order);
            if (!risk.approved) {
                publish('paper_order_rejected', { order, risk }, 'risk');
                return json(res, 422, { ok: false, risk });
            }
            const trade = openPaperTrade(order.productId, order.side, order.size, order.price);
            publish('paper_order_opened', { trade, risk }, 'paper');
            return json(res, 201, { ok: true, trade, risk });
        }
        if (path === '/api/agents/run' && req.method === 'POST') {
            const body = await readJson(req);
            const agentId = String(body.agentId || ''), asset = String(body.asset || 'UNKNOWN'), summary = String(body.summary || '');
            if (!agentId || !summary)
                return json(res, 400, { error: 'agentId and summary are required.' });
            publish('agent_started', { asset }, agentId);
            const result = await runAgent(agentId, asset, summary);
            saveAnalysis(agentId, asset, summary, result.output, result.model);
            publish('agent_completed', { asset, output: result.output }, agentId);
            return json(res, 200, result);
        }
        if (path === '/api/agents/pipeline/status' && req.method === 'GET')
            return json(res, 200, getPipelineStatus());
        if (path === '/api/scanner' && req.method === 'GET')
            return json(res, 200, await scanCryptoMarket());
        if (path === '/api/agents/auto-run' && req.method === 'GET')
            return json(res, 200, getAutoRunSettings());
        ...
        if (path === '/api/challenge' && req.method === 'GET')
            return json(res, 200, await getChallengeSnapshot());
        ...
        if (path === '/api/risk/settings' && req.method === 'GET')
            return json(res, 200, getRuntimeRiskLimits());
        ...
        if (path === '/api/live/readiness' && req.method === 'GET') {
            if (!coinbaseConfigured())
                return json(res, 503, { error: 'Coinbase credentials are not configured.' });
        ...
        if (path === '/api/quant/status' && req.method === 'GET')
            return json(res, 200, await quantStatus());
        if (path === '/api/quant/vectorbt/sma' && req.method === 'POST') {
            const body = await readJson(req);
            const result = await runVectorbtSma(body);
            publish('vectorbt_backtest_completed', { result }, 'strategy');
            return json(res, 200, { ok: true, result });
        }
        if (path === '/api/quant/nautilus/smoke' && req.method === 'POST') {
            const result = await runNautilusSmoke();
            publish('nautilus_engine_ready', { result }, 'backtest');
            return json(res, 200, { ok: true, result });
        }
        if (path === '/api/quant/rdagent/health' && req.method === 'GET') {
            const result = await runRdAgent('health');
            return json(res, 200, { ok: true, result });
        }
        if (path === '/api/quant/rdagent/run' && req.method === 'POST') {
            const body = await readJson(req);
            const command = String(body.command || 'fin_quant');
            if (!['fin_quant', 'fin_factor', 'health', 'info'].includes(command))
                return json(res, 400, { error: 'Invalid RD-Agent command.' });
            const stepN = Math.max(1, Math.min(5, Number(body.stepN) || 1)), loopN = Math.max(1, Math.min(5, Number(body.loopN) || 1));
            const result = await runRdAgent(command, stepN, loopN);
            publish('rdagent_run_completed', { command, stepN, loopN }, 'strategy');
            return json(res, 200, { ok: true, result });
        }
        if (path === '/api/coinbase/status' && req.method === 'GET')
            return json(res, 200, { configured: coinbaseConfigured(), portfolioUuid: config.coinbasePortfolioUuid || null });
        if (path === '/api/coinbase/diagnostics' && req.method === 'GET')
            return json(res, 200, coinbaseCredentialShape());
        if (path === '/api/coinbase/accounts' && req.method === 'GET') {
            if (!coinbaseConfigured())
                return json(res, 503, { error: 'Coinbase credentials are not configured.' });
            return json(res, 200, { accounts: await listAccounts() });
        }
        if (path.startsWith('/api/coinbase/product/') && req.method === 'GET') {
            if (!coinbaseConfigured())
                return json(res, 503, { error: 'Coinbase credentials are not configured.' });
            const productId = decodeURIComponent(path.split('/').pop() || 'BTC-USD').toUpperCase();
            return json(res, 200, { product: await getProduct(productId) });
        }
        if (path.startsWith('/api/coinbase/candles/') && req.method === 'GET') {
            if (!coinbaseConfigured())
                return json(res, 503, { error: 'Coinbase credentials are not configured.' });
            const productId = decodeURIComponent(path.split('/').pop() || 'BTC-USD').toUpperCase();
            const limit = Math.max(20, Math.min(300, Number(url.searchParams.get('limit')) || 60));
            return json(res, 200, { candles: await getCandles(productId, 'ONE_HOUR', limit) });
        }
        if (path.startsWith('/api/coinbase/order-book/') && req.method === 'GET') {
            if (!coinbaseConfigured())
                return json(res, 503, { error: 'Coinbase credentials are not configured.' });
            const productId = decodeURIComponent(path.split('/').pop() || 'BTC-USD').toUpperCase();
            return json(res, 200, { book: await getProductBook(productId, 8) });
        }
        if (path.startsWith('/api/coinbase/trades/') && req.method === 'GET') {
            if (!coinbaseConfigured())
                return json(res, 503, { error: 'Coinbase credentials are not configured.' });
            const productId = decodeURIComponent(path.split('/').pop() || 'BTC-USD').toUpperCase();
            return json(res, 200, { trades: await getMarketTrades(productId, 12) });
        }
...
const shutdown = () => { stopAutoRun(); try {
