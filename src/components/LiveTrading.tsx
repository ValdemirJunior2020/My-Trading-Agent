      ...
      <section className="panel live-panel">
        ...
        {dailyGuard ? (
          <div className={dailyGuard.blocked ? 'daily-guard-detail blocked' : 'daily-guard-detail'}>
            <div><small>{pt ? 'Início do dia' : 'Start of day'}</small><strong>{'
          <div>
...
} + Number(dailyGuard.startEquityUsd || 0).toFixed(2)}</strong></div>
            <div><small>{pt ? 'Agora' : 'Current'}</small><strong>{'
          <div>
            <small>{pt ? 'Portfólio atual' : 'Current portfolio'}</small>
            <strong>
              {readiness?.currentPortfolioUsd != null
...
} + Number(dailyGuard.currentEquityUsd || 0).toFixed(2)}</strong></div>
            <div><small>{pt ? 'Perda do dia' : 'Daily loss'}</small><strong>{Number(dailyGuard.lossPercent || 0).toFixed(2) + '%'}</strong></div>
            <div><small>{pt ? 'Limite' : 'Limit'}</small><strong>{Number(dailyGuard.limitPercent || 0).toFixed(2) + '%'}</strong></div>
          </div>
        ) : null}
